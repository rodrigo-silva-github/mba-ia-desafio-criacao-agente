"""Verificação da Fase 4 (storage de negócio) — sem chave de API.

Uso:  uv run python -m aurora.scripts.verificar_storage

Cobre: restauração do estado inicial, CRUD de reservas e visitantes,
unicidade de códigos (incluindo os cancelados), exclusividade área+data no
...
async def cenario_seed() -> None:
    ...
    reservas_101 = await reservas_store.list_apartment("101")
    check(len(reservas_101) == 1 and reservas_101[0]["codigo"] == "RSV-1377", "101 tem só a RSV-1377")
    reservas_302 = await reservas_store.list_apartment("302")
    check(len(reservas_302) == 1 and reservas_302[0]["codigo"] == "RSV-4821", "302 tem só a RSV-4821")
    visitantes_302 = await visitantes_store.list_apartment("302")
    check(
        len(visitantes_302) == 1 and visitantes_302[0]["nome"] == "Marina Duarte",
        "302 tem Marina Duarte",
    )
    check(await reservas_store.is_available("quadra", "2030-03-09") is False, "quadra 2030-03-09 ocupada")
    check(await reservas_store.is_available("quadra", "2030-04-06") is True, "quadra 2030-04-06 livre")


async def cenario_crud() -> None:
    print("\n== CRUD ==")
    r = await reservas_store.book("101", "quadra", "2030-04-06")
    check(r.ok and r.reservation and r.reservation.codigo.startswith("RSV-"), "reserva nova criada")
    assert r.reservation
    codigo1 = r.reservation.codigo

    r2 = await reservas_store.book("201", "quadra", "2030-04-06")
    check(not r2.ok and r2.reason == "date_taken", "dup área+data recusada (Garantia 5)")

    check(await reservas_store.cancel("101", codigo1) is True, "cancelou a própria reserva")
    check(await reservas_store.cancel("302", codigo1) is False, "não cancela reserva alheia (G2)")

    r3 = await reservas_store.book("101", "quadra", "2030-04-06")
    check(r3.ok and r3.reservation and r3.reservation.codigo != codigo1, "código novo difere do cancelado")
    assert r3.reservation
    lista = [x["codigo"] for x in await reservas_store.list_apartment("101")]
    check(codigo1 not in lista, "reserva cancelada não aparece na lista")

    v = await visitantes_store.authorize("101", "Joana Ribeiro", "2030-04-21")
    check(v["nome"] == "Joana Ribeiro" and v["data"] == "2030-04-21", "visitante autorizado")
    nomes = [x["nome"] for x in await visitantes_store.list_apartment("101")]
...
async def cenario_concorrencia() -> None:
    ...
    async def tenta(apartamento: str) -> dict:
        res = await reservas_store.book(apartamento, "salao-de-festas", "2030-05-11")
        return {"ok": res.ok, "codigo": res.reservation.codigo if res.reservation else None}
    ...
    async def tenta_b(apartamento: str, area: str) -> dict:
        res = await reservas_store.book(apartamento, area, "2030-06-01")
        return {"ok": res.ok, "codigo": res.reservation.codigo if res.reservation else None}
    ...
    todas = await reservas_store.all_active()
...
async def main() -> int:
    ...
    for p in (config.BUSINESS_DB_PATH, config.BUSINESS_DB_PATH + "-wal", config.BUSINESS_DB_PATH + "-shm"):
        if os.path.exists(p):
            os.remove(p)
    await db.init_db()
